...
-- basic configuration
local block_size = 4096*1024 -- Block size 256k
local backend = "http://" .. ngx.var.host .. "/" -- backend
...
if matches then
	if matches[1] == nil and matches[2] then
		...
		-- stop = matches[2] or (origin_headers["Content-Length"] - 1) Must be able to reply partial request 0-0
		stop = matches[2] or 0
	end
else
	stop = (origin_headers["Content-Length"] - 1)
